...
use crate::{components::LocalStorage, game::{Board, BoardPos, Card, DECK_SIZE, DepotRole, NUM_RANKS, NUM_SUITS, RANKS, SettingsState, Skin, Suit}};
...
impl GameState {
    ...
    pub fn undo(&mut self) {
        ...
        while self.history.len() > target_len {
            ...
            self.board.do_move(rec.pos2, rec.pos1, rec.tap.map(|x| !x));
            if rec.tap == Some(true) {self.board.cheats_delta -= 1;}
